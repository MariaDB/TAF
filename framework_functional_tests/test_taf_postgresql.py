...
Layers:
    ...
    L4  Sysbench build via `taf.pl --action=build-client` — requires cmake + libpq
    L5  Full benchmark run (short); builds its own client via
        init-start-db-build-client-run-tests, independent of L4
    L6  MariaDB regression test — requires TAF_MARIADB_DIR
    L7  Sysbench-lua test-type sweep against PostgreSQL
    L8  HammerDB TPROC-C against PostgreSQL
    L9  HammerDB TPROC-H against PostgreSQL

Prerequisites:
    sudo bash framework_functional_tests/setup_almalinux10.sh

Running:
    export TAF_PG_INSTALL_DIR=/usr/pgsql-16   # default
    export TAF_PG_PORT=5433                   # default, avoids conflict
    python3 -m pytest framework_functional_tests/test_taf_postgresql.py -v
...
def benchmark_result(tmp_path_factory):
    ...
    # *-build-client-run-tests: cmake builds exactly one driver per
    # invocation (WITH_PGSQL xor WITH_MYSQL), so this fixture rebuilds the
    # client itself against the active PostgreSQL install rather than
    # assuming a persistent binary L4 (or L6, targeting MariaDB) left behind.
    write_props(props_file, {
        "taf.action":                     "init-start-db-build-client-run-tests",
        "taf.taf_db_makers_plugin":       "postgres",
        "taf.db_software_install_dir":    str(PG_INSTALL),
        "taf.db_port":                    str(PG_PORT),
        "taf.db_user":                    TAF_PG_USER,
...
# HammerDB job-tracking SQLite files. Fixed, un-namespaced names under /tmp,
# shared by every OS user and every target DB (not just PostgreSQL) -- see
# client_source/hammerdb/HammerDB-6.0/config/*.xml for the per-database ones;
# hammer.DB is the main Jobs database. Whichever user creates them first owns
# them, and /tmp's sticky bit means a *different* user can neither write to
# nor delete them afterwards -- reproducibly "attempt to write a readonly
# database" for that other user (e.g. running the suite as root, then again
# as a regular user, or vice versa, on the same box).
_HAMMERDB_TMP_STATE = [
    "hammer.DB", "generic.db", "database.db", "ci.db", "db2.db",
    "mariadb.db", "mssqlserver.db", "mysql.db", "oracle.db", "postgresql.db",
]


def _clean_hammerdb_tmp_state() -> None:
    """Best-effort, idempotent removal of HammerDB's shared /tmp state.

    Safe to call unconditionally: missing files are a silent no-op, and a
    file owned by a different OS user (which this process has no permission
    to remove, sticky-bit /tmp) is skipped rather than raising -- the most
    this can do for that case is not be the one leaving stale state behind
    for the *next* run, which is why it runs at both fixture setup and
    teardown, not just one or the other.
    """
    for name in _HAMMERDB_TMP_STATE:
        try:
...
# ===========================================================================
# LAYER 4 — Sysbench client build via TAF's own build-client action
# ===========================================================================
#
# Sysbench is a cmake project inside this repo (client_source/sysbench-lua/
# CMakeLists.txt) and TAF builds it through its own mechanism --
# `perl taf.pl --action=build-client`, which drives
# libs/script_tools_lib/ClientCmakeBuild.pm. L4 exercises exactly that
# action instead of invoking autogen.sh/configure/make directly, so a
# regression in TAF's own build-client path is what actually gets caught
# here (and was: ClientCmakeBuild.pm's install-dir maker detection
# misclassified every Percona PostgreSQL install as MariaDB-family, because
# "percona-postgresql-18.4-..." matches the ambiguous "percona" token before
# ever trying "postgresql").
#
# cmake builds exactly one driver per invocation (WITH_PGSQL xor WITH_MYSQL,
# mutually exclusive in ClientCmakeBuild.pm) -- there is no persistent
# dual-driver binary. L5/L6/L7 each rebuild the client themselves for their
# own target via the *-build-client-run-tests action, so this layer's own
# build is not reused by anything downstream; it is a standalone check.

SYSBENCH_BUILD_PROPS = {
    "taf.action":                  "build-client",
    "taf.taf_db_makers_plugin":    "postgres",
    "taf.test_suite":              "sysbench-lua",
    "taf.db_software_install_dir": None,   # filled in per-test
    "sysbench_lua.db_driver":      "pgsql",
    "sysbench_lua.cmake_args":     "-DCMAKE_BUILD_TYPE=Release",
}


class TestL4SysbenchBuild:
    """Verifies that `perl taf.pl --action=build-client` builds sysbench
    with the pgsql driver against the active PostgreSQL install."""

    SYSBENCH_BIN = TAF_ROOT / "client_source" / "sysbench-lua" / "sysbench"

    @needs_pg
    def test_build_client_action_succeeds(self, tmp_path_factory):
        tmp = tmp_path_factory.mktemp("taf_l4")
        props_file = tmp / "build_client.properties"
        props = dict(SYSBENCH_BUILD_PROPS)
        props["taf.db_software_install_dir"] = str(PG_INSTALL)
        write_props(props_file, props)

        result = taf_propfile(props_file, timeout=180)
        assert result.returncode == 0, (
            f"build-client failed (rc={result.returncode})\n"
            f"STDOUT:\n{result.stdout[-4000:]}\nSTDERR:\n{result.stderr[-1000:]}"
        )

    @needs_pg
    def test_sysbench_binary_exists_after_build(self):
        assert self.SYSBENCH_BIN.is_file(), \
            f"Sysbench binary not found: {self.SYSBENCH_BIN}"
        assert os.access(str(self.SYSBENCH_BIN), os.X_OK), \
            f"Sysbench binary is not executable: {self.SYSBENCH_BIN}"

    @needs_pg
    def test_sysbench_pgsql_driver_available(self):
        """Sysbench must support --db-driver=pgsql."""
        if not self.SYSBENCH_BIN.is_file():
            pytest.skip("Sysbench binary not found — run L4 build test first")

        result = run(str(self.SYSBENCH_BIN), "--db-driver=pgsql", "--help", timeout=15)
        output = result.stdout + result.stderr
        has_pgsql = any(
            keyword in output.lower()
        ...
        assert has_pgsql, (
            "Sysbench does not support pgsql driver after build-client\n"
            f"Output: {output[:500]}"
        )
...
class TestL6MariaDBRegression:
    ...
    def test_mariadb_run_exits_cleanly(self, tmp_path_factory):
        ...
        write_props(props_file, {
            # init-start-db-build-client-run-tests (not start-db-run-tests):
            # this must work from a freshly-provisioned MariaDB install with
            # no pre-existing initialized data dir -- start-db-run-tests
            # alone assumes the system tables already exist and just hangs
            # waiting for readiness otherwise. build-client rebuilds sysbench
            # for the mysql driver: cmake links exactly one driver per
            # invocation, so the pgsql-driver binary L4/L5 may have left
            # behind cannot be reused here.
            "taf.action":                  "init-start-db-build-client-run-tests",
